...
class BotInstance:
    ...
    def close_position(self):
        ...
        try:
            ...
            # Configurar el tipo de mercado correcto
            ...
            # Determinar el lado para cerrar la posición
            side = "sell" if self.current_position["side"] == "long" else "buy"

            # Parámetros específicos según exchange y tipo de mercado
            order_params = {}

            # Obtener información sobre el par
            try:
                market = self.exchange.markets[self.symbol]
                contract_size = None
                lot_size = None

                # Para OKX específicamente
                if self.exchange.id == 'okx' and self.market_type == 'future':
                    if 'contractSize' in market:
                        contract_size = safe_float(market['contractSize'], 1.0)
                    # También buscar en info.lotSz (específico de OKX)
                    if 'info' in market and 'lotSz' in market['info']:
                        lot_size = safe_float(market['info']['lotSz'], None)
                    ...
                    # Ajustar cantidad al tamaño de lote si es necesario
                    amount = self.current_position["amount"]
                    if lot_size:
                        amount = round(amount / lot_size) * lot_size
                        logger.info(f"[{self.symbol}] Ajustando cantidad para cierre a múltiplo de lot_size: {amount}")

                    # Configurar parámetros para OKX
                    order_params['tdMode'] = 'cross'

                    # Para OKX SWAP, usar el formato correcto
                    if 'SWAP' not in self.symbol:
                        order_params['instId'] = f"{base_currency}-{quote_currency}-SWAP"

                    # Para posiciones cortas, usar USDT como moneda de margen
                    if self.current_position["side"] == "short":
                        order_params['ccy'] = quote_currency

            except Exception as e:
                logger.warning(f"[{self.symbol}] Error al obtener información de mercado para cierre: {str(e)}")

            # Lógica específica por exchange
            if self.market_type == "future":
                if self.exchange.id == 'binance':
                    order_params['reduceOnly'] = True
                elif self.exchange.id != 'okx':  # Para otros exchanges que no son OKX
                    order_params['reduceOnly'] = True

            logger.info(
                f"[{self.symbol}] Cerrando posición: {side} {self.current_position['amount']} {self.symbol} con parámetros: {order_params}")
            ...
            logger.info(f"[{self.symbol}] Posición cerrada con éxito, P&L: {pnl:.2f}%")
            return trade_record

        except Exception as e:
            logger.error(f"[{self.symbol}] Error al cerrar posición: {str(e)}")
            import traceback
    ...
    def generate_signal(self, df):
        ...
        if self.current_position:
            ...
            # Señal de compra (largo) - válida tanto para spot como futuros
    ...
    def execute_signal(self, signal, df):
        ...
        try:
            # Si la señal es cerrar, primero cerrar posición existente
            if signal["action"] == "close" and self.current_position:
                ...
                return  # Salir después de cerrar posición

            # Si la señal es comprar o vender, verificar que no haya posición abierta
            elif signal["action"] in ["buy", "sell"] and not self.current_position:
                # Verificar si el tipo de mercado es compatible con la acción
                if signal["action"] == "sell" and self.market_type != "future":
                    logger.warning(f"[{self.symbol}] Se intentó abrir posición corta en mercado spot. Ignorando señal.")
                    return
                ...
                # Calcular stop loss
                stop_loss = entry_price - (atr * atr_multiplier) if signal["action"] == "buy" else entry_price + (
                        atr * atr_multiplier)

                # Calcular take profit
                take_profit = entry_price * (1 + take_profit_pct / 100) if signal[
                                                                               "action"] == "buy" else entry_price * (
                ...
                # Obtener mercado y sus requisitos específicos
                try:
                    market = self.exchange.markets[self.symbol]

                    # Obtener información sobre el tamaño de lote
                    min_amount = safe_float(market.get('limits', {}).get('amount', {}).get('min', 0), 0)
                    amount_precision = None

                    # Intentar obtener precision (cantidad de decimales) del monto
                    if 'precision' in market:
                        if isinstance(market['precision'], dict) and 'amount' in market['precision']:
                            amount_precision = market['precision']['amount']
                        elif isinstance(market['precision'], (int, float)):
                            amount_precision = int(market['precision'])

                    # Para OKX específicamente, tratar de obtener el contractSize (tamaño del contrato)
                    contract_size = None
                    lot_size = None
                    if self.exchange.id == 'okx' and self.market_type == 'future':
                        if 'contractSize' in market:
                    ...
                    logger.info(
                        f"[{self.symbol}] Datos de mercado: min_amount={min_amount}, precision={amount_precision}, contract_size={contract_size}, lot_size={lot_size}")
                except Exception as e:
                    logger.warning(f"[{self.symbol}] Error al obtener información de mercado: {str(e)}")
                    min_amount = 0
                    amount_precision = None
                    contract_size = None
                    lot_size = None

                # Calcular cantidad basada en el precio de entrada
                ...
                # NUEVO: Ajustar al tamaño de lote específico para OKX
                original_amount = amount

                if self.exchange.id == 'okx' and self.market_type == 'future':
                    # OKX futuros necesita múltiplos exactos del tamaño de lote
                    if lot_size:
                        # Redondear al múltiplo más cercano del lotSize
                        amount = round(amount / lot_size) * lot_size
                        logger.info(
                            f"[{self.symbol}] Cantidad ajustada a múltiplo de lot_size {lot_size}: {original_amount} → {amount}")
                    elif contract_size:
                        # Alternativa: usar contractSize
                        amount = round(amount / contract_size) * contract_size
                        logger.info(
                            f"[{self.symbol}] Cantidad ajustada a múltiplo de contract_size {contract_size}: {original_amount} → {amount}")
                else:
                    # Redondear según precisión del exchange para otros casos
                    try:
                        amount = float(self.exchange.amount_to_precision(self.symbol, amount))
                        logger.info(f"[{self.symbol}] Cantidad redondeada por exchange: {original_amount} → {amount}")
                    except Exception as e:
                        # Si falla, intentar redondear según la precisión conocida
                        if amount_precision is not None:
                            amount = round(amount, amount_precision)
                        else:
                            # Último recurso: redondear a 8 decimales
                            amount = round(amount, 8)
                        logger.warning(
                            f"[{self.symbol}] Error al redondear cantidad con exchange: {str(e)}. Redondeando manualmente a {amount}")
                ...
                if amount < min_amount and min_amount > 0:
                    ...
                    if min_amount > 0:
                        amount = min_amount
                        logger.info(f"[{self.symbol}] Ajustando a cantidad mínima: {amount}")
                    else:
                        return
                ...
                try:
                    ...
                    # Configurar apalancamiento primero si es mercado de futuros
                    if self.market_type == "future" and hasattr(self, 'leverage'):
                        ...
                        try:
                            if self.exchange.id == 'binance':
                                ...
                                logger.info(f"[{self.symbol}] Configurado apalancamiento en Binance: {leverage_int}x")
                            elif self.exchange.id == 'okx':
                                # Para OKX, configurar el apalancamiento
                                self.exchange.set_leverage(leverage_int, self.symbol)
                                logger.info(f"[{self.symbol}] Configurado apalancamiento en OKX: {leverage_int}x")
                            else:
                                # Método genérico para otros exchanges
                                self.exchange.set_leverage(leverage_int, self.symbol)
                                logger.info(f"[{self.symbol}] Configurado apalancamiento: {leverage_int}x")
                        except Exception as e:
                            logger.warning(f"[{self.symbol}] No se pudo configurar apalancamiento: {str(e)}")

                    # Configuración específica por exchange para la creación de órdenes
                    if self.market_type == "future":
                        if self.exchange.id == 'okx':
                            # Para OKX en futuros
                            order_params['tdMode'] = 'cross'  # Modo de trading

                            # Extraer monedas base y cotizada del par
                            if '/' in self.symbol:
                                base_currency, quote_currency = self.symbol.split('/')
                            else:
                                # Intentar inferir del formato usado por OKX (ETH-USDT-SWAP)
                                parts = self.symbol.split('-')
                                if len(parts) >= 2:
                                    base_currency, quote_currency = parts[0], parts[1]
                                else:
                                    base_currency = self.symbol
                                    quote_currency = 'USDT'  # Valor por defecto

                            # Para OKX SWAP, el formato correcto es ETH-USDT-SWAP
                            if 'SWAP' not in self.symbol:
                                order_params['instId'] = f"{base_currency}-{quote_currency}-SWAP"
                                logger.info(f"[{self.symbol}] Usando instId específico: {order_params['instId']}")

                            # Usar USDT como moneda de margen para posiciones cortas
                            if signal["action"] == "sell":
                                order_params['ccy'] = quote_currency

                        elif self.exchange.id == 'binance':
                            # Configuración para Binance futuros (si necesario)
                            pass
